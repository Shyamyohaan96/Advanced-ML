...
from tensorflow.keras.layers import SimpleRNN, Dense, LeakyReLU, Input, GRU, Embedding, Conv1D, MaxPooling1D, Dropout,LSTM, Bidirectional
...
def RNN_CNN_network(n_tokens,len_max,ins_train,outs_train,spa_accuracy,
                   n_neurons, 
                   activation, 
                   activation_dense,
                   cnn_filter,
    ...
    for i in range(len(cnn_filter)):
        model.add(Conv1D(filters=cnn_filter[i], kernel_size=cnn_kernel[i],activation=activation_dense))
    ...
    for i in range(len(dense_unit)):
        model.add(Dense(units=dense_unit[i],
...
def create_parser():
    ...
    parser = argparse.ArgumentParser(description='RNN_CNN', fromfile_prefix_chars='@')
...
if __name__ == "__main__":
    ...
    #building the model
    model = RNN_CNN_network(dat_out['n_tokens'],
                            dat_out['len_max'],
                            dat_out['ins_train'],
                            dat_out['outs_train'],
                            spa_accuracy, 
    ...
    # Callbacks
    early_stopping_cb = keras.callbacks.EarlyStopping(patience=30,restore_best_weights=True,min_delta=0.01)
        
        
    #fitting the model 
    history = model.fit(x=dat_out['ins_train'],y=dat_out['outs_train'],batch_size=32,epochs=100,steps_per_epoch = 50,validation_data=(dat_out['ins_valid'],dat_out['outs_valid']),callbacks=[early_stopping_cb])
    ...
    #plot model
    plot_model(model, to_file='%s_model_plot_%d.png'%(args.name,args.rotation), show_shapes=True, show_layer_names=True)
    
    #dumping the outputs into pickel files
    fname1 = '%s_historyn_%d.pkl'%(args.name,args.rotation)
    fname2 = '%s_evaluationn_%d.pkl'%(args.name,args.rotation)
