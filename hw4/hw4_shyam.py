...
#################################################################
def inception_layer(x,filters_1x1, filters_3x3_reduce, filters_3x3, filters_3x3_reduce_2, filters_3x3_2,filters_pool,name, lambda_l2):
    con_1X1 = Conv2D(filters_1x1, 
                     (3, 3), 
                     strides = 1,
                     padding = 'valid',
                     use_bias = True,
                     ...
                     activation = 'elu',
                     kernel_regularizer = lambda_l2)(x)
    
    con_3X3 = Conv2D(filters_3x3_reduce, 
                     (1, 1), 
                     strides = 1,
                     padding = 'valid',
                     use_bias = True,
    ...
    con_3X3 = Conv2D(filters_3x3, 
                     ...
                     bias_initializer = 'zeros',
                     activation = 'elu',
                     kernel_regularizer = lambda_l2)(con_3X3)
    
    con_3X3_2 = Conv2D(filters_3x3_reduce_2, 
                     (1, 1), 
                     strides = 1,
                     padding = 'valid',
                     use_bias = True,
    ...
    con_3X3_2 = Conv2D(filters_3x3_2, 
                     (3, 3), 
                     strides = 1,
                     padding = 'valid',
                     use_bias = True,
                     ...
                     bias_initializer = 'zeros',
                     activation = 'elu',
                     kernel_regularizer = lambda_l2)(con_3X3_2)
    
    pool = MaxPooling2D(pool_size =(3,3),
                        strides = (1,1))(x)
    
    pool = Conv2D(filters_pool, 
                  (1, 1), 
                  strides = 1,
                  padding = 'valid',
                  use_bias = True,
                  ...
                  bias_initializer = 'zeros',
                  activation = 'elu',
                  kernel_regularizer = lambda_l2)(pool)
    
    inception = concatenate([con_1X1, con_3X3, con_3X3_2, pool], axis=3, name=name)
    
    return inception
    

def create_cnn_classifier_network(image_size, nchannels, conv_layers, dense_layers,inception_layers, p_dropout, lambda_l2, lrate,
                                  n_classes):
    ...
    #model = Sequential();
    #input
    Input_layer = Input(shape =(image_size[0], image_size[1], nchannels))
    new = Conv2D(conv_layers[0]['filters'], 
                         conv_layers[0]['kernel_size'], 
                         strides = 1,
                         padding = 'valid',
                         use_bias = True,
                         kernel_initializer = 'random_uniform',
                         bias_initializer = 'zeros',
                         name = 'Convolution_'+str(0),
                         activation = 'elu',
                         kernel_regularizer = lambda_l2)(Input_layer)
    new = MaxPooling2D(pool_size =conv_layers[0]['pool_size'],
                                   strides = conv_layers[0]['strides'])(new)
    
    # convolution layer
    for i in range(1, len(conv_layers)):
        new = Conv2D(conv_layers[i]['filters'], 
                         ...
                         activation = 'elu',
                         kernel_regularizer = lambda_l2)(new)
        #Pooling layer
        if conv_layers[i]['pool_size'] is not None:
            new = MaxPooling2D(pool_size =conv_layers[i]['pool_size'],
                                   strides = conv_layers[i]['strides'])(new)
    #inception layer
    l = 0
    for i in inception_layers:
        new = inception_layer(new,
                         filters_1x1=i[0],
                         filters_3x3_reduce=i[1],
                         filters_3x3=i[2],
                         filters_3x3_reduce_2=i[3],
                         filters_3x3_2=i[4],
                         filters_pool=i[5],
                         name='inception_'+str(l+1),
                         lambda_l2 = lambda_l2)
        l = l+1
        new = SpatialDropout2D(p_dropout)(new)
    ...
    for i in range(len(dense_layers)):
        new_1 = Dense(units = dense_layers[i]['units'],
                        use_bias = True,
                        kernel_initializer = 'random_uniform',
                        bias_initializer = 'zeros',
                        name = 'Dense_'+str(i+1),
                        activation = 'elu',
                        kernel_regularizer = lambda_l2)(new_1)
        new_1 = Dropout(p_dropout)(new_1)
    
    #output layer
    new_1 = Dense(units = n_classes ,
    ...
    model = Model(Input_layer, new_1, name='inception_1')
...
def create_parser():
    ...
    parser.add_argument('--rotation_range', type=int, default=30, help='Rotation range')
    parser.add_argument('--horizontal_flip', type=bool, default=True, help='horizontal_flip')
    ...
    #inception layers
    parser.add_argument('--No_inception', type=int, default=1, help='Number of inception layers')
    parser.add_argument('--inception_filter', nargs='+', type=int, default=[64, 64, 128,32,64,64], help='Inception filters')
...
def execute_exp(args=None):
    ...
    train_generator=ImageDataGenerator(rescale=1./255., rotation_range=args.rotation_range, horizontal_flip=args.horizontal_flip)
    ...
    li = args.inception_filter
    x = int(len(li)/args.No_inception)
    final_list= lambda li, x: [li[i:i+x] for i in range(0, len(li), x)]

    inception_layers=final_list(li, x)
